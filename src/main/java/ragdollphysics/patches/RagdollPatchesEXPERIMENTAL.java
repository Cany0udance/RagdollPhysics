...
public class RagdollPatchesEXPERIMENTAL {
    ...
    private static void initializeRagdoll(AbstractMonster monster) {
        try {
            ...
            MultiBodyRagdoll ragdoll =
                    new MultiBodyRagdoll(skeleton, monster.drawY, monster.drawX,
                            monster.drawY, monster.getClass().getSimpleName(), monster);
    ...
    private static void initializeImageRagdoll(AbstractMonster monster) {
        try {
            MultiBodyRagdoll ragdoll =
                    new MultiBodyRagdoll(monster.drawX, monster.drawY, monster.drawY,
    ...
    // Updated renderRagdoll method - passes monster to renderDetachedAttachments
    private static void renderRagdoll(AbstractMonster monster, SpriteBatch sb,
        ...
        if (monster.tint.color.a > 0) {
            ...
            if (atlas == null) {
                ...
                // UPDATED: Pass monster to renderDetachedAttachments
    ...
    public static class MultiBodyRagdoll {
        ...
        public MultiBodyRagdoll(Skeleton skeleton, float groundLevel, float startX,
                                float startY, String monsterClassName, AbstractMonster monster) {
            ...
            for (Slot slot : skeleton.getSlots()) {
                if (slot.getAttachment() != null) {
                    ...
                    boolean shouldDetach = AttachmentConfig.shouldDetachAttachment(
                            monsterClassName, attachmentName);
                    BaseMod.logger.info("DEBUG: Attachment '" + attachmentName
                            + "' should detach: " + shouldDetach);
        ...
        public MultiBodyRagdoll(float startX, float startY, float groundLevel,
            ...
            // Store the offset between monster draw position and physics body start
            // position
            this.initialOffsetX = monster.drawX - startX;
            this.initialOffsetY = monster.drawY - startY;
        ...
        public void applyGlobalForce(float forceX, float forceY) {
            ...
            for (AttachmentPhysics attachment : attachmentBodies.values()) {
                ...
                attachment.velocityX +=
                        forceX * MathUtils.random(0.5f, 1.2f); // Original: (0.8f, 1.0f)
                attachment.velocityY +=
                        forceY * MathUtils.random(0.4f, 1.0f); // Original: (0.6f, 0.8f)

                // INCREASED: Angular velocity range
                float attachmentBaseAngular =
                        MathUtils.random(-360f, 360f); // Original: (-180f, 180f)
                attachment.angularVelocity +=
                        attachmentBaseAngular * (1.0f + flipIntensity * 0.5f);

                // INCREASED: Random variation
                attachment.velocityX +=
                        MathUtils.random(-75f, 75f); // Original: (-25f, 25f)
                attachment.velocityY +=
                        MathUtils.random(-50f, 100f); // Original: (-15f, 35f)
        ...
        public void renderDetachedAttachments(
                PolygonSpriteBatch sb, TextureAtlas atlas, AbstractMonster monster) {
            ...
            for (Map.Entry<String, AttachmentPhysics> entry :
                ...
                // Apply monster's tint color directly to attachments for synchronized
                // fading
                    ...
                    try {
                        ...
                        // Handle MeshAttachment
                            ...
                            MeshAttachment meshAttachment =
                                    (MeshAttachment) attachmentPhysics.attachment;
                            TextureAtlas.AtlasRegion region =
                                    (TextureAtlas.AtlasRegion) meshAttachment.getRegion();
    ...
    public static class AttachmentPhysics {
        ...
        // NEW: Fade-out properties
        private float lifeTimer = 0f;
        private float alpha = 1.0f;
        private static final float FADE_START_TIME =
                3.0f; // Start fading after 3 seconds
        private static final float FADE_DURATION = 1.0f; // Fade over 1 second
        ...
        public AttachmentPhysics(float startX, float startY, float groundLevel,
            ...
            // Remove all fade-related initialization
        ...
        public void update(float deltaTime) {
            ...
            // REMOVE all fade logic - just do physics
